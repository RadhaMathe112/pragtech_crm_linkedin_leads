...
class SocialLinkedinController(http.Controller):

    @fragment_to_query_string
    @http.route(['/social_linkedin_leads/callback'], type='http', auth='user')
    def linkedin_account_token_callback(self, access_token=None, is_extended_token=False, **kw_social):
        ...
        if access_token:
            media = request.env.ref('pragtech_crm_linkedin_leads.linkedin_pragtech_social_media')

            try:
                self._create_linkedin_accounts(access_token, media, is_extended_token)
                return "You can Close this window now"

            except (AccessError, MissingError):
                pass

    def _create_linkedin_accounts(self, access_token, media, is_extended_token):
        extended_access_token = access_token if is_extended_token else self._get_linkedin_access_token(access_token,
                                                                                                        media)
